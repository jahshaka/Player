...
bool Database::updateProjectGuid(const QString &newGuid, const QString &guid)
{
    QSqlQuery query;
    query.prepare("UPDATE projects SET guid = ? WHERE guid = ?");
    query.addBindValue(newGuid);
    query.addBindValue(guid);
    return executeAndCheckQuery(query, "UpdateProjectGuid");
}
...
QVector<ProjectTileData> Database::fetchProjects()
{
    ...
    while (query.next())  {
        ...
        if (data.guid != "00000000-0000-0000-0000-00reserved00") tileData.push_back(data);
    }
...
{
    QSqlQuery query;
    query.prepare("SELECT name, thumbnail, guid, date_created FROM projects WHERE guid = ?");
    query.addBindValue(guid);
    executeAndCheckQuery(query, "FetchProjects");
    ...
    while (query.next()) {
        ...
        data.thumbnail = record.value(1).toByteArray();
        data.guid = record.value(2).toString();
        data.dateCreated = record.value(3).toDateTime();
        tileData = data;
    }
    return tileData;
}
