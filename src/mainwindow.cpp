...
#include "irisgl/src/zip/src/zip.h"
...
iris::ScenePtr MainWindow::createDefaultScene()
{
    ...
    //auto path = IrisUtils::getAbsoluteAssetPath("app/content/skies/alternative/space");
    //auto ext = "png";
    ...
    //scene->skyBoxTextures[0] = z1;
    //scene->skyBoxTextures[1] = z2;
    //scene->skyBoxTextures[2] = y1;
    //scene->skyBoxTextures[3] = y2;
    //scene->skyBoxTextures[4] = x1;
    //scene->skyBoxTextures[5] = x2;

    //auto img = new QImage(z1);

    //scene->setSkyTexture(iris::Texture2D::createCubeMap(z1, z2, y1, y2, x1, x2, img));
    //scene->setSkyTextureSource(z1);
    //scene->setSkyColor(QColor(255, 255, 255));
    ...
    //node->setLocalScale(QVector3D(8, 1, 4));
    //node->setLocalRot(QQuaternion::fromEulerAngles(QVector3D(90, 0, 0)));
    //node->setGlobalPos(QVector3D(0, 0, 0));

    //auto node2 = iris::MeshNode::create();
    //node2->setMesh(":/content/primitives/plane1.obj");
    //scene->rootNode->addChild(node2);
    //node2->setLocalScale(QVector3D(8, 4, 1));
    //node2->setLocalRot(QQuaternion::fromEulerAngles(QVector3D(0, 40, 0)));
    //node2->setGlobalPos(QVector3D(-9.8, 0, 1));

    //auto node3 = iris::MeshNode::create();
    //node3->setMesh(":/content/primitives/plane1.obj");
    //scene->rootNode->addChild(node3);
    //node3->setLocalScale(QVector3D(8, 4, 1));
    //node3->setLocalRot(QQuaternion::fromEulerAngles(QVector3D(0, -40, 0)));
    //node3->setGlobalPos(QVector3D(9.8, 0, 1));
    ...
    //mat2->addTexture("u_matTexture", sceneView->metadataController->texture);
    ...
    //mat3->addTexture("u_matTexture", sceneView->infoController->texture);
    //node3->setMaterial(mat3);


    //auto node12 = iris::ViewerNode::create();
    //scene->rootNode->addChild(node12);
    //node12->setGlobalPos(QVector3D(0, 0, 15));
    ...
    return scene;
    //return iris::ScenePtr();
}

void MainWindow::createLobbyScene()
{
	//auto yaw = sceneView->editorCam->getLocalRot().toEulerAngles().y();
	//auto baseRot = QQuaternion::fromEulerAngles(0, yaw, 0);
	float dist = 5;
    auto basePos = QVector3D(0, 5, 0); // sceneView->editorCam->getLocalPos();

	root = iris::SceneNode::create();
	root->setLocalPos(basePos);
    ...
    //root->addChild(node5);
    //node5->setLocalScale(QVector3D(10, 1, 5) / 5);
    //node5->setLocalRot(QQuaternion::fromEulerAngles(QVector3D(deckRot, 0, 0)));
    //node5->setLocalPos(QVector3D(0, -5, 0.33 - dist));
    //node5->setPickable(false);
    //node5->pickingGroups = 1 << 5;
    ...
    //mat5->setValue("diffuseColor", QColor(32, 32, 32));
    //mat5->setValue("diffuseTexture", QDir(Globals::project->getProjectFolder()).filePath("Tile.png"));
    //mat5->setValue("textureScale", 4.f);
    //node5->setMaterial(mat5);
...
void MainWindow::initializeGraphics(SceneViewWidget *widget, QOpenGLFunctions_3_2_Core *gl)
{
    Q_UNUSED(gl);
    setupVrUi();
    sceneView->createOverlayController();
}
...
void MainWindow::openProject(bool playMode)
{
    ...
    if (QFileInfo(Globals::project->getProjectFolder()).baseName() == "00000000-0000-0000-0000-00reserved00") {
        createLobbyScene();
    }
...
void MainWindow::setupDesktop()
{
	...
	connect(pmContainer, SIGNAL(startVirtualLobby()), SLOT(createVirtualLobby()));
	connect(pmContainer, SIGNAL(exportProject()), SLOT(exportSceneAsZip()));

    // if the project already exists don't import it
    if (!db->checkIfRecordExists("guid", "00000000-0000-0000-0000-00reserved00", "projects")) {
        qDebug() << "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@running importer";
        pmContainer->importProjectFromStore(IrisUtils::getAbsoluteAssetPath("/data/lobby.zip"));
    }
    else {
...
void MainWindow::createVirtualLobby()
{
    if (UiManager::isSceneOpen) closeProject();
