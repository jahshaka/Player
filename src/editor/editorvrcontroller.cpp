...
void EditorVrController::enableMovement(bool enable)
{
...
void EditorVrController::update(float dt)
{
    ...
    if (leftTouch->isTracking()) {
		...
		else
		{
            // Handle picking and movement of picked objects
            iris::PickingResult pick;
            if (rayCastToScene(leftHandRenderItem->worldMatrix, pick, true)) {
                auto dist = qSqrt(pick.distanceFromStartSqrd);
	...
	if (rightTouch->isTracking()) {
		...
		*/
			...
			// Handle picking and movement of picked objects
			iris::PickingResult pick;
			if (rayCastToScene(rightHandRenderItem->worldMatrix, pick, true)) {
				auto dist = qSqrt(pick.distanceFromStartSqrd);
...
bool EditorVrController::rayCastToScene(QMatrix4x4 handMatrix, iris::PickingResult& result, bool all)
{
