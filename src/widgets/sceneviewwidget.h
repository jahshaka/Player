...
#include <QTimer>
#include "../vr/overlaycontroller.h"
...
#include "vr/overlaylistmodel.h"

#include "vr/overlaycontroller.h"
#include "vr/overlayimageprovider.h"
...
class SceneViewWidget : public QOpenGLWidget, protected QOpenGLFunctions_3_2_Core
{
    ...
    iris::MeshNodePtr floatingMenuNode;

public:   
    void setSharedContext(QOpenGLContext *sharedCtx) {
        QOpenGLContext::currentContext()->setShareContext(sharedCtx);
    }
    ...
    QOpenGLContext *getCurrentContext() {
     ...
     // MOVE ALL OF THESE TO THEIR OWN CLASSES LATER ON
     OverlayListModel *libraryModel;
     OverlayListModel *onlineModel;

     // rename this to something better
     SwipeManager *swipeManager;
     OverlayMethodHandler *overlayMethodHandler;

     OverlayListModel *hoveredModel;
     QStringList metadataModel;
     OverlayImageProvider *colorImageProvider;
     OverlayNetworkManager *networkManager;

     OverlayController *controller;
     OverlayController *controlController;
     OverlayController *metaController;
     OverlayController *viewController;
     //OverlayController *handController;

     QMap<long, OverlayController*> vrControllers;

     void createOverlayController() {
        //controller = new OverlayController(this);
        //TODO REMOVE
        controller->kickOff("qrc:/qml/MainView.qml");
        metaController->kickOff("qrc:/qml/MetadataView.qml");
        controlController->kickOff("qrc:/qml/ControlView.qml");
        viewController->kickOff("qrc:/qml/Deck.qml");
        //handController->kickOff("qrc:/qml/handmen.qml");
     }

     void initializeVRControllers();

     void acceptQMLFramebuffer(QOpenGLContext *share, uint texture) {
        if (texture) {
            qDebug() << texture;
        }
     }
...
private:
    ...
    QPointF doMenuPicking(const QVector3D& segStart, const QVector3D& segEnd, long &nodeId);

	void updateCamControllerVr();
	QPointF prevVrPoint;
