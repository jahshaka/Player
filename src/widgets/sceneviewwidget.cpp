...
#include "vr/overlaynetworkmanager.h"

#include "vr/overlaymethodhandler.h"
#include "uimanager.h"
...
void SceneViewWidget::initializeVRControllers()
{
    SwipeManager *swipeManager = new SwipeManager();

    libraryModel = new OverlayListModel;
    onlineModel = new OverlayListModel;
    swipeManager = new SwipeManager;

    networkManager = new OverlayNetworkManager;

    colorImageProvider = new OverlayImageProvider;
    colorImageProvider->setDatabase(database);

    overlayMethodHandler = new OverlayMethodHandler(swipeManager);
    overlayMethodHandler->setDatabase(database);

    controller = new OverlayController(this, QSize(1920, 1080));
    controller->attachExternalObject("libraryModel", libraryModel);
    controller->attachExternalObject("onlineModel", onlineModel);
    controller->attachExternalObject("swipeManager", swipeManager);
    controller->attachExternalObject("methodHandler", overlayMethodHandler);
    controller->attachExternalObject("hoveredModel", metadataModel);
    controller->getQmlEngine()->addImageProvider(QLatin1String("guids"), colorImageProvider);

    overlayMethodHandler->controller = controller;

    for (const auto &project : database->fetchProjects()) {
        libraryModel->addScene(OverlayScene(project.name, project.guid));
    }

    networkManager->makeRequest();

    metaController = new OverlayController(this, QSize(400, 800));
    metaController->attachExternalObject("methodHandler", overlayMethodHandler);
    metaController->attachExternalObject("swipeManager", swipeManager);
    metaController->getQmlEngine()->addImageProvider(QLatin1String("guids"), colorImageProvider);

    controlController = new OverlayController(this, QSize(400, 800));
    controlController->attachExternalObject("swipeManager", swipeManager);
    controlController->attachExternalObject("methodHandler", overlayMethodHandler);

    viewController = new OverlayController(this, QSize(1366, 200));
    viewController->attachExternalObject("methodHandler", overlayMethodHandler);
    viewController->attachExternalObject("swipeManager", swipeManager);

    {
        //connect(overlayMethodHandler, &OverlayMethodHandler::worldDownloadProgress,
        //    onlineModel, &OverlayListModel::receiveSceneDownloadProgress);
 /*       connect(overlayMethodHandler, &OverlayMethodHandler::worldDownloadComplete,
            onlineModel, &OverlayListModel::receiveSceneCompletionProgress);*/

        connect(networkManager, &OverlayNetworkManager::sceneFetched, this, [&](OverlayScene &scene) {
            //todo mr lazlo - check if this is already in the library
            //qDebug() << "API " << scene.name() << scene.guid();

            // don't need to request a special image from the image provider, it uses the featured_image url as the key
            //QSize size;
            //colorImageProvider->requestImage(scene.featured_image(), &size, QSize());
            scene.setInLibrary(database->checkIfRecordExists("guid", scene.actual_guid(), "projects"));
            onlineModel->addScene(scene);
        });

        // connect to the download signal to add tile to pane when dloaded
        connect(overlayMethodHandler, &OverlayMethodHandler::sceneFetched, this, [this](QString guid) {
            auto project = database->fetchProject(guid);
            libraryModel->addScene(OverlayScene(project.name, project.guid));
        });
    }
}
...
SceneViewWidget::SceneViewWidget(QWidget *parent) : QOpenGLWidget(parent)
{
	...
	defaultCam->disableMovement();
	defaultCam->disableZoom();
	defaultCam->disablePan();
...
void SceneViewWidget::setScene(iris::ScenePtr scene)
{
    ...
    //{
    //    floatingMenuNode.reset();

    //    floatingMenuNode = iris::MeshNode::create();
    //    floatingMenuNode->hide();
    //    floatingMenuNode->setMesh(":/content/primitives/plane1.obj");
    //    scene->rootNode->addChild(floatingMenuNode);
    //    floatingMenuNode->setPickable(false);
    //    floatingMenuNode->pickingGroups = 1 << 5;

    //    auto mat = iris::CustomMaterial::create();
    //    mat->generate(IrisUtils::getAbsoluteAssetPath("/app/shader_defs/Widget.shader"));
    //    mat->renderStates.rasterState = iris::RasterizerState::createCullNone();
    //    mat->addTexture("u_matTexture", handController->texture);
    //    floatingMenuNode->setMaterial(mat);
    //}
}
...
QPointF SceneViewWidget::doMenuPicking(const QVector3D& segStart, const QVector3D& segEnd, long &nodeId)
{
    QList<iris::PickingResult> hitList;
	//doScenePicking(scene->getRootNode(), segStart, segEnd, hitList);
	scene->rayCast(scene->getRootNode(), segStart, segEnd, hitList, (uint64_t)PickingFlags::Menu, true);
	...
	qSort(hitList.begin(), hitList.end(), [](const iris::PickingResult& a, const iris::PickingResult& b) {
        return a.distanceFromStartSqrd > b.distanceFromStartSqrd;
    });

    auto pickedNode = hitList.last().hitNode;
    auto pickedPoint = hitList.last().hitPoint;

    nodeId = pickedNode->getNodeId();

    auto hitter = pickedNode->getGlobalTransform().inverted() * pickedPoint;// *QVector3D(1280, 800, 0);

    auto width = vrControllers[nodeId]->dimensions.width();
    auto height = vrControllers[nodeId]->dimensions.height();

    QPointF points((width + (hitter.x() * width)) / 2.f, (height + (hitter.z() * height)) / 2.f);

    return points;
}

void SceneViewWidget::updateCamControllerVr()
{
	auto vrDevice = vrPublicDevice = iris::VrManager::getDefaultDevice();
	auto rightTouch = vrDevice->getTouchController(1);

	if (camController == vrCam) {
		auto segStart = vrCam->rightBeamRenderItem->worldMatrix * QVector3D(0, 0, 0);
		auto segEnd = vrCam->rightBeamRenderItem->worldMatrix * QVector3D(0, 0, -1000);


        long nodeId = 0;
		auto point = this->doMenuPicking(segStart, segEnd, nodeId);
		if (point != QPointF(0, 0)){
			if (point != prevVrPoint && nodeId != 0) {
				QMouseEvent mappedEvent(QEvent::MouseMove, point, Qt::NoButton, Qt::NoButton, Qt::NoModifier);
				QApplication::instance()->sendEvent(vrControllers[nodeId]->getWindow(), &mappedEvent);
				prevVrPoint = point;
			}

			if (rightTouch->isIndexTriggerPressed()) {
				QMouseEvent mappedEvent(QEvent::MouseButtonPress, point, Qt::LeftButton, Qt::NoButton, Qt::NoModifier);
				QApplication::instance()->sendEvent(vrControllers[nodeId]->getWindow(), &mappedEvent);
			}

			if (rightTouch->isIndexTriggerReleased()) {
				QMouseEvent mappedEvent(QEvent::MouseButtonRelease, point, Qt::LeftButton, Qt::NoButton, Qt::NoModifier);
				QApplication::instance()->sendEvent(vrControllers[nodeId]->getWindow(), &mappedEvent);
			}
		}

        //if (rightTouch->isButtonDown(iris::VrTouchInput::B)) {
        //    QMatrix4x4 world;
        //    world.setToIdentity();
        //    world.translate(0, .25f, -.1f);

        //    world = vrCam->leftHandRenderItem->worldMatrix * world;

        //    floatingMenuNode->setLocalTransform(world);
        //    QVector3D baseSize(5 * .1f, 3 * .1f, 1);
        //    floatingMenuNode->setLocalScale(QVector3D(baseSize.x(), baseSize.y(), 1.f));

        //    floatingMenuNode->show();
        //}

        //if (rightTouch->isButtonUp(iris::VrTouchInput::B)) {
        //    floatingMenuNode->hide();
        //}
	}

	if (vrDevice->isHeadMounted()) {
		if (rightTouch->isButtonPressed(iris::VrTouchInput::A)) {
			//if (!!mainWindow->root) {
			//    mainWindow->removeLobbyScene();
			//}
			//else {
			//    mainWindow->createLobbyScene();
			//}
...
void SceneViewWidget::mouseMoveEvent(QMouseEvent *e)
{
...

    {
		...
		auto segStart = screenSpaceToWoldSpace(e->localPos(), -1.0f);
		auto segEnd = screenSpaceToWoldSpace(e->localPos(), 1.0f);

        long nodeId = 0;
        auto point = this->doMenuPicking(segStart, segEnd, nodeId);
        if (point != QPointF(0, 0)) {
            QMouseEvent mappedEvent(e->type(), point, e->button(), e->buttons(), e->modifiers());
            QCoreApplication::sendEvent(vrControllers[nodeId]->getWindow(), &mappedEvent);
        }
    }
...
void SceneViewWidget::mouseReleaseEvent(QMouseEvent *e)
{
...

    {
        ...
        if (point != QPointF(0, 0)) {
            QMouseEvent mappedEvent(e->type(), point, e->button(), e->buttons(), e->modifiers());
			//qDebug() << e->type() << e->button() << e->button() << e->buttons() << e->modifiers();
            QApplication::instance()->sendEvent(vrControllers[nodeId]->getWindow(), &mappedEvent);
        }
    }
