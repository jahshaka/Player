...
ProjectManager::ProjectManager(Database *handle, QWidget *parent) : QWidget(parent), ui(new Ui::ProjectManager)
{
    ...
    ui->pmContainer->setStyleSheet(
		"border: none;"
		"background-image: url(:/images/empty_canvas_player.png);"
...
void ProjectManager::openProjectFromWidget(ItemGridWidget *widget, bool playMode)
{
	...
	Globals::project->setProjectPath(
        QDir(QDir(projectFolder).filePath("Scenes")).filePath(widget->tileData.guid),
...
void ProjectManager::openProjectFromGuid(const QString &guid)
{
    ...
    Globals::project->setProjectPath(
        QDir(QDir(projectFolder).filePath("Scenes")).filePath(guid),
        "Random World Identifier"
    );
...
void ProjectManager::importProjectFromDownload(const QString & guid)
{
    importProjectFromFile(guid);
}

void ProjectManager::importProjectFromDownloadSilently(const QString & str, const QString &guid)
{
    qDebug() << "DFR " << str << guid;
    importProjectFromFile(str, true, guid);
}

void ProjectManager::importProjectFromStore(const QString & guid)
{
    importProjectFromFileVR(guid);
}
...
void ProjectManager::importProjectFromFileVR(const QString& file)
{
    QString fileName;
    if (file.isEmpty()) {
    ...
    auto pDir = QDir(QDir(defaultProjectDirectory).filePath("Scenes")).filePath("00000000-0000-0000-0000-00reserved00");
    ...
    db->updateProjectGuid("00000000-0000-0000-0000-00reserved00", importGuid);
}

void ProjectManager::importProjectFromFile(const QString& file, bool silent, const QString &preserveGuid)
{
    QString fileName;
    if (file.isEmpty()) {
        fileName = QFileDialog::getOpenFileName(this,       "Import Scene",
    ...
    // now extract the project to the default projects directory with the name
    auto importGuid = preserveGuid.isEmpty() ? GUIDManager::generateGUID() : preserveGuid;
    auto pDir = QDir(QDir(defaultProjectDirectory).filePath("Scenes")).filePath(importGuid);
    zip_extract(fileName.toStdString().c_str(), pDir.toStdString().c_str(), Q_NULLPTR, Q_NULLPTR);
...
void ProjectManager::deleteProjectFromWidget(ItemGridWidget *widget)
{
    ...
    if (option == QMessageBox::Yes) {
        QDir dirToRemove(QDir(projectFolder + "/Scenes").filePath(widget->tileData.guid));
